...
coords <- read.csv("final.coords", sep = " ", header = FALSE, stringsAsFactors = FALSE)
...
coords_lgl <- coords
all(rownames(coords_lgl) == V(g1)$names)
...
# Save image
png(file="ALL_Clusters_ilgl2.png", width=1280, height=800)
par(bg = "black")
plot(g1, 
     layout = coords_igraph_lgl, 
     vertex.size = 0, 
     vertex.color = NA, 
